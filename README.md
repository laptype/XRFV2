...
After downloading the dataset, please note that the **XRFV2** folder contains the data used during training, which is split into training and testing datasets, as specified in `basic_config.json` under the `dataset_path` parameter. The **XRFV2_processed** folder contains the raw, unsegmented data, which is intended for testing and evaluation, and corresponds to the `dataset_root_path` in `basic_config.json`. 

Once you have downloaded the dataset, make sure to update the paths in `basic_config.json` accordingly.
